...
window.selectLectureForPlacement = function(lecture, isBreak) {
    if (!isMobile) return false; // PC는 드래그앤드롭 사용

    // 이미 같은 강의 선택 중 → 취소
    if (AppState.selectedLectureForPlacement && AppState.selectedLectureForPlacement.id === lecture.id) {
        cancelTapToPlace();
        return true;
    }
    ...
    const selectedItem = document.querySelector(`.lecture-item[data-lecture-id="${lecture.id}"]`);
    if (selectedItem) selectedItem.classList.add('tap-selected');

    // 토스트 표시
    showPlacementToast(lecture.titleKo);
...
/**
 * 셀 탭 → handleDrop 로직 실행
 */
window.executeTapToPlace = function(cell) {
    const lecture = AppState.selectedLectureForPlacement;
    if (!lecture) return;

    // handleDrop 재사용: draggedLecture/draggedIsBreak 세팅 후 호출
    AppState.draggedLecture = lecture;
    AppState.draggedIsBreak = AppState.selectedLectureIsBreak;
    AppState.draggedScheduleKey = null;

    // handleDrop은 this = cell 을 기대하므로 call 사용
    handleDrop.call(cell, { preventDefault: () => {}, target: cell });
...
function showPlacementToast(title) {
    hidePlacementToast();
    ...
    el.innerHTML = `<span>📌 <b>${shortTitle}</b> 배치 중…&nbsp;시간표에서 셀을 선택하세요</span>
        <button onclick="cancelTapToPlace()" style="background:rgba(255,255,255,0.2);border:none;color:white;border-radius:50%;width:22px;height:22px;cursor:pointer;font-size:0.8rem;flex-shrink:0;">✕</button>`;
    document.body.appendChild(el);
    _placementToastEl = el;
}

function hidePlacementToast() {
    if (_placementToastEl) {
        _placementToastEl.remove();
        _placementToastEl = null;
    }
    const existing = document.getElementById('tapPlacementToast');
